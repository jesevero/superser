...
export default function AdminCategoriasScreen() {
  ...
  const { auth } = useAuth();
  const { contextoId, contextoNome } = useLocalSearchParams<{ contextoId: string; contextoNome: string }>();
  ...
  async function loadData() {
    const { data } = await supabase.from("categorias").select("*").eq("contexto_id", contextoId).order("ordem");
    setCategorias(data || []);
    setLoading(false);
  }
  ...
  function openEdit(cat: Categoria) {
    setEditingId(cat.id);
    setNome(cat.nome);
    setOrdem(cat.ordem != null ? String(cat.ordem) : "");
    setMessage(null);
    setModalVisible(true);
  }
  ...
  async function handleSave() {
    ...
    if (editingId) {
      const { error } = await supabase.from("categorias").update(row).eq("id", editingId);
      if (error) { setMessage({ text: `Erro: ${error.message}`, error: true }); setSaving(false); return; }
    } else {
      row.contexto_id = contextoId;
      const { error } = await supabase.from("categorias").insert(row);
      if (error) { setMessage({ text: `Erro: ${error.message}`, error: true }); setSaving(false); return; }
    }
  ...
  function handleTap(cat: Categoria) {
    router.push(`/admin/indicadores?categoriaId=${cat.id}&categoriaNome=${encodeURIComponent(cat.nome)}&contextoId=${contextoId}`);
  }
  ...
  return (
    <View style={styles.container}>
      <ScrollView contentContainerStyle={styles.content}>
        <Text style={styles.contextLabel}>{contextoNome}</Text>
        <View style={styles.header}>
          <Text style={styles.title}>{categorias.length} categoria(s)</Text>
        ...
        {categorias.map((cat) => (
          <TouchableOpacity key={cat.id} style={styles.card} onPress={() => handleTap(cat)} activeOpacity={0.7}>
            <View style={styles.cardInfo}>
              <Text style={styles.cardName}>{cat.nome}</Text>
              {cat.ordem != null ? <Text style={styles.cardDate}>Ordem: {cat.ordem}</Text> : null}
            </View>
            ...
            {confirmDelete === cat.id ? (
              <View style={styles.confirmRow}>
                <TouchableOpacity onPress={() => handleDelete(cat.id)} style={styles.confirmYes}>
              ...
              <TouchableOpacity onPress={() => setConfirmDelete(cat.id)} style={styles.iconBtn}>
